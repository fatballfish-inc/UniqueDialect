...
var (
	...
	normalizeSetSessionTxReadOnlyAssignmentPattern = regexp.MustCompile(`(?is)^\s*SET\s+(?:(?:SESSION\s+|@@session\.))?(?:tx_read_only|transaction_read_only)\s*=\s*(?:[01]|ON|OFF|TRUE|FALSE)\s*;?\s*$`)
)
